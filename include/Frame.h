...
#define FRAME_GRID_COLS 64
    ...
    public:
        ...
        std::vector<double> EpiErr;
        std::vector<double> epiErr;
        std::vector<int> cal_f_num;
        bool isInBox(cv::KeyPoint &kp,YoloBoundingBox &Box,bool extend = false);
        void CalcEpiDistChi2(const std::vector<cv::Point2f> &kp1, const std::vector<cv::Point2f> &kp2,cv::Mat &F, std::vector<uchar> state);
        void key_box_dep(const std::vector<cv::Point2f> &kp1, const std::vector<cv::Point2f> &kp2, 
        cv::Mat &F, const cv::Mat imDepthScaled, YoloBoundingBox &it);

    //planar
        bool depthisInBox(int x, int y, YoloBoundingBox &Box);
        bool lineisInBox(cv::line_descriptor::KeyLine &mvkeyline,YoloBoundingBox &Box,bool extend = false);
        void reduceVector(std::vector<cv::Point2f> &v, std::vector<bool> status);
        void RemoveMovingKeyPoints(const cv::Mat &ImGray, const cv::Mat &ImDepth);
        void RemoveMovingKeyPoints1(const cv::Mat &ImGray, const cv::Mat &ImDepth);
        void EpiloarWithFundMatrix(const cv::Mat &ImGrayPre, const cv::Mat &ImGray, const cv::Mat &imDepthScaled);
        void CalcEpiDistChi2(const std::vector<cv::KeyPoint> &kp1, const std::vector<cv::Point2f> &kp2,cv::Mat &F, std::vector<uchar> state);
        void RemoveMovingPointsWithEpiAndYOLO(std::vector<double> epiErr,
                                             const cv::Mat &ImGray, const cv::Mat &ImDepth);
   
        vector<cv::Rect2f> mvDynamicArea;
        //line
        void reduceLineVector(std::vector<cv::Point2f> &s, std::vector<bool> status_s, std::vector<cv::Point2f> &e, std::vector<bool> status_e);

        //ovd add
        std::vector<cv::KeyPoint> mvKeysDynamic;
        std::vector<float> mvDynamicProbability;
        std::vector<cv::line_descriptor::KeyLine> mvLineDynamic;
        cv::Mat mDescriptorsDynamic;
        cv::Mat mlbdDynamic;
        //ovd add 
        Tracking* mpTracker;
        ...
        Frame(Tracking* pTracker,  const cv::Mat &imRGB, const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp,
        ...
        void ExtractORB(const cv::Mat &im, const cv::Mat &mask = cv::Mat());

        // Build the semantic-suspect / Manhattan-immunity prior. Pixels on large
        // depth planes are exempted from a coarse detection box.
        cv::Mat BuildStaticMask(const cv::Size &size, const cv::Mat &depth) const;
        void EvaluateDynamicProbabilities(const cv::Mat &currentGray,
                                          const cv::Mat &previousGray,
                                          const cv::Mat &depth,
                                          const cv::Mat &manhattanImmunityMask);
