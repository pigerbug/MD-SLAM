...
namespace ORB_SLAM2 {
    ...
    public:
         ...
         //ovd add
        std::vector<double> EpiErr_tmp;
        std::vector<double> fenbu_tmp;
        std::vector<int> cal_f_num_track;
        cv::Mat mK;
        cv::Mat semantic_mask;
        cv::Mat show_semantic_mask, prev_semantic_id, semantic_id;
        cv::Mat compen_semantic, finial_img;
        ...
        //ovd add
        void SetYoloDetector(YoloDetector* yolodetect);
        void GetImg(const cv::Mat &img, const cv::Mat &depth);

        //ovd add
        bool depthisInBox(int x, int y, YoloBoundingBox &Box);
        void setSemanticMask(int x1, int y1, int x2, int y2, const cv::Mat& depth_img, bool is_compensate = false);
        void compensateMissedDetection(int x, int y,const cv::Mat& depth_img);
        unsigned short Set_Rect_depth(int x1, int y1, int x2, int y2, const cv::Mat& depth);
