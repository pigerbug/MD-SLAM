...
bool PlaneDetection::readDepthImage(Tracking* mpTracker, const cv::Mat depthImg, const cv::Mat &K, const float &depthMapFactor) {
    ...
    cv::Mat imDepthScaled;
        if (depthMapFactor != 1 || depth_img.type() != CV_32F) {
            depth_img.convertTo(imDepthScaled, CV_32F, depthMapFactor);
        }
    ...
    for (int i = 0; i < rows; i += 3) {
        for (int j = 0; j < cols; j += 3) {
            ...
            //float chi = (imDepthScaled.at<float>(i,j) - it->key_dep_average) / it->key_dep_stdcov;
            if( mpTracker->semantic_mask.at<unsigned short>(i,j)!=0 )
			{
                
               
                //mpTracker->show_semantic_mask.at<unsigned short>(i,j)!=0 && 
                //cout<<"here is dynamic plannar pointcloud"<<"show_semantic_mask: "<<mpTracker->show_semantic_mask.at<unsigned short>(i,j)<<" compen_semantic: "<<mpTracker->compen_semantic.at<unsigned short>(i,j)<<endl;
				//cloud.vertices[vertex_idx++] = VertexType(0,0, std::numeric_limits<double>::quiet_NaN());
                //cloud.vertices[vertex_idx++] = VertexType(0, 0, z);
				continue;
			}
