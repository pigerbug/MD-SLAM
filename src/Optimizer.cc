...
namespace ORB_SLAM2 {
    ...
    int Optimizer::PoseOptimization(Frame *pFrame) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (pMP) {
                    ...
                    if (pFrame->mvuRight[i] < 0) {
                        ...
                        const double staticWeight = i < pFrame->mvDynamicProbability.size() ?
                            std::max(0.05, 1.0 - static_cast<double>(pFrame->mvDynamicProbability[i])) : 1.0;
                        e->setInformation(Eigen::Matrix2d::Identity() * invSigma2 * staticWeight);
                    ...
                    } else  // Stereo observation
                    {
                        ...
                        const double staticWeight = i < pFrame->mvDynamicProbability.size() ?
                            std::max(0.05, 1.0 - static_cast<double>(pFrame->mvDynamicProbability[i])) : 1.0;
                        Eigen::Matrix3d Info = Eigen::Matrix3d::Identity() * invSigma2 * staticWeight;
        ...
        // When point observations are scarce, Manhattan-aligned line endpoints
        // become the high-confidence directional fallback in the joint graph.
        const double lineDensityWeight = nInitialCorrespondences < 80 ? 4.0 :
                                         (nInitialCorrespondences < 150 ? 2.0 : 1.0);
        ...
        // Set MapLine vertices
        {
            ...
            for (int i = 0; i < NL; i++) {
                ...
                if (pML) {
                    ...
                    els->setInformation(Eigen::Matrix3d::Identity() * lineDensityWeight);
                    ...
                    ele->setInformation(Eigen::Matrix3d::Identity() * lineDensityWeight);
    ...
    int Optimizer::TranslationOptimization(ORB_SLAM2::Frame *pFrame) {
        ...
        const double translationLineDensityWeight = nInitialCorrespondences < 80 ? 4.0 :
                                                    (nInitialCorrespondences < 150 ? 2.0 : 1.0);

        {
            ...
            for (int i = 0; i < NL; i++) {
                ...
                if (pML) {
                    ...
                    els->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(0)));
                    els->setMeasurement(line_obs);
                    els->setInformation(Eigen::Matrix3d::Identity() * translationLineDensityWeight);
                    ...
                    ele->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(0)));
                    ele->setMeasurement(line_obs);
                    ele->setInformation(Eigen::Matrix3d::Identity() * translationLineDensityWeight);
